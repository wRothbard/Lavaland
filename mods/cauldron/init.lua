...
function cauldron.filling(pos, node, clicker, itemstack)
	...
	if wield_item:sub(1,7) == "bucket:" then
		if wield_item:sub(-6) == "_empty" and not (node.name:sub(-6) == "_empty") then
			...
			minetest.set_node(pos, {name = "cauldron:cauldron_empty", param2 = node.param2})

		elseif wield_item:sub(-6) == "_water" and node.name:sub(-6) == "_empty" then
			minetest.set_node(pos, {name = "cauldron:cauldron_idle", param2 = node.param2})
...
function cauldron.idle_timer(pos)
	...
	minetest.set_node(pos, {name = "cauldron:cauldron_boiling", param2 = node.param2})
	return true
end
...
function cauldron.boiling_timer(pos)
	...
	if #ingredients >= 2 then
		...
		minetest.set_node(pos, {name = "cauldron:cauldron_soup", param2 = node.param2})
	end
...
function cauldron.take_soup(pos, node, clicker, itemstack)
	...
	if item_name == "cauldron:bowl" then
		if wield_item:get_count() > 1 then
			if inv:room_for_item("main", "cauldron:bowl_soup 1") then
				itemstack:take_item()
				inv:add_item("main", "cauldron:bowl_soup 1")
			else
				minetest.chat_send_player(clicker:get_player_name(),
			...
			itemstack:replace("cauldron:bowl_soup 1")
		end
...
minetest.register_node("cauldron:cauldron_empty", {
...
minetest.register_node("cauldron:cauldron_idle", {
	groups = {cracky=2, oddly_breakable_by_hand=1, not_in_creative_inventory=1},
	on_rotate = screwdriver.rotate_simple,
	tiles = {"xdecor_cauldron_top_idle.png", "xdecor_cauldron_sides.png"},
	drop = "cauldron:cauldron_empty",
...
minetest.register_node("cauldron:cauldron_boiling", {
	groups = {cracky=2, oddly_breakable_by_hand=1, not_in_creative_inventory=1},
	on_rotate = screwdriver.rotate_simple,
	drop = "cauldron:cauldron_empty",
...
minetest.register_node("cauldron:cauldron_soup", {
	groups = {cracky = 2, oddly_breakable_by_hand = 1, not_in_creative_inventory = 1},
	on_rotate = screwdriver.rotate_simple,
	drop = "cauldron:cauldron_empty",
...
minetest.register_craft({
	output = "cauldron:cauldron_empty",
	recipe = {
		{"steel:iron_lump", "", "steel:iron_lump"},
		{"steel:iron_lump", "", "steel:iron_lump"},
		{"steel:iron_lump", "steel:iron_lump", "steel:iron_lump"}
	}
})

print("loaded cauldron")
