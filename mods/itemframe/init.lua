...
local function remove_item(pos, node)
	...
	for _, obj in pairs(objs) do
		...
		if obj and ent and ent.name == "itemframe:f_item" then
			obj:remove() break
		end
	end
end
...
local function update_item(pos, node)
	...
	local entity = minetest.add_entity(pos, "itemframe:f_item")
...
minetest.register_node("itemframe:itemframe", {
	...
	node_box = {
		type = "fixed",
		fixed = {
			-0.5, -0.5, -0.5 + 0.9375, 0.5, 0.5, 0.5
		}
	},
	tiles = {
...
minetest.register_entity("itemframe:f_item", {
	visual = "wielditem",
	...
	on_activate = function(self, staticdata)
		...
		if minetest.get_node(pos).name ~= "itemframe:itemframe" then
			self.object:remove()
		end
...
minetest.register_craft({
	output = "itemframe:itemframe",
	recipe = {
		{"group:stick", "group:stick", "group:stick"},
		{"group:stick", "papyrus:paper", "group:stick"},
		{"group:stick", "group:stick", "group:stick"}
	}
})

print("loaded itemframe")
