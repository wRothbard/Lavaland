-- mobs are part of Glitchtest
-- Glitchtest is Copyright 2018 James Stevenson
...
local log = function(message, name)
	if not debug then
		return
	end
...
local erase = function(pos)
	...
	if not node then
		--log("erase: node not found, returning")
		return
	end

	local node_name = node.name
	if not node_name then
		--log("erase: node_name not found, returning")
		return
	end
...
-- Limit spawning if area is already populated
local limiter = function(pos, radius, limit, immediate_surrounding, surrounding)
	...
	if #immediate_surrounding > 6 then
		log("limiter: return true, found > 6 in immediate_surrounding")
		return true
	end
	...
	if #surrounding > 18 then
		...
		if h > limit then
			log("limiter: return true, h > limit")
			return true
		end
	end
	--log("limiter: finish")
...
local spawner_limiter = function(pos, node, active_object_count, active_object_count_wider)
	...
	--log("spawner_limiter: finish")
end

local check_for_player = function(pos, radius)
	--log("check_for_player: begin")
	radius = radius or 32
	...
	for i = 1, #objects_in_radius do
		...
		if player then
			--log("check_for_player: return true, found player")
			return true
		end
	end
	--log("check_for_player: finish")
end
...
local redo = function(pos, radius)
	...
	if n then
		...
		if not t:is_started() then
			t:start(0)
			--log("redo: timer not started, restarting")
		end
	end
...
on_timer = function(pos, elapsed)
	...
	if elapsed < 12 then
		...
		--log("on_timer: elapsed < 12, returning")
		return
	end

	if check_for_player(pos) then
		timer:start(0)
		--log("on_timer: nearby player found, restarting timer")
		return
	end
	...
	if s["air"] < d then
		--log("on_timer: returning redo(pos)")
		return redo(pos)
	end

	minetest.add_entity(spawn_pos, mob)
	erase(pos)
	log("on_timer: finish, spawned " .. mob)
end

local function undercrowd(pos, radius)
	--log("undercrowd: begin")
	radius = radius or 8
	...
	for _, v in pairs(r) do
		...
		if t > 5 then
			v:remove()
			log("undercrowd: removed " .. s.name)
		end
	end
	--log("undercrowd: finish, returning " .. tostring(t))
	return t
end

-- play sound
local mob_sound = function(self, sound, gain, pitch, delay)
	if sound then
		local gain = gain or random(60, 93) / 100 + random() / 11
		local pitch = pitch or 0.96 + random(1, 7) / 100 + random() / 11
		local sh = minetest.sound_play(sound, {
			gain = self.sounds.gain or gain,
			pitch = pitch,
			object = self.object,
			max_hear_distance = self.sounds.distance or 64,
		})
		delay = delay or 0.5
		minetest.after(delay, minetest.sound_fade, sh, -1, 0.0)
	end
end
...
local set_animation = function(self, anim)
	...
	if anim ~= "punch" and anim ~= "shoot" and
			string.find(self.animation.current, anim) then
		return
	end
	-- check for more than one animation
	local num = 0
	for n = 1, 4 do
		if self.animation[anim .. n .. "_start"] and
				self.animation[anim .. n .. "_end"] then
			num = n
		end
	end
	...
	if anim == self.animation.current or
			not self.animation[anim .. "_start"] or
			not self.animation[anim .. "_end"] then
		return
	end
	self.animation.current = anim
	self.object:set_animation({x = self.animation[anim .. "_start"],
...
local line_of_sight = function(self, pos1, pos2, stepsize)
	...
	while minetest.registered_nodes[nn] and
			(minetest.registered_nodes[nn].walkable == false) do
			--or minetest.registered_nodes[nn].drawtype == "nodebox") do
		npos1 = vector.add(npos1, stepv)
		if get_distance(npos1, pos2) < stepsize then
			return true
		end
		-- scan again
...
local flight_check = function(self, pos_w)
	...
	if def.drawtype ~= "airlike" and def.drawtype ~= "liquid" and
			def.drawtype ~= "flowingliquid" then
		return true
	end
	return false
end
...
local is_at_cliff = function(self)
	...
	if nn and nn.name then
		...
		if nndef and nndef.drawtype and
			...
			--print("Found liquid below.")
			set_yaw(self, yaw + random(-0.75, 0.75), 1)
			self.state = "walk"
			return true
		end
	end
...
local do_env_damage = function(self)
	...
	if self.light_damage ~= 0 then
		...
		if light >= self.light_damage_min and
				light <= self.light_damage_max then
			self.health = self.health - self.light_damage
			effect(pos, 5, "tnt_smoke.png")
			if check_for_death(self, {type = "light"}) then
				return
			end
		end
	end
...
--	or self.standing_in == node_permanent_flame) then
		if self.lava_damage ~= 0 then
			...
			if check_for_death(self, {type = "environment",
					pos = pos, node = self.standing_in, hot = true}) then
				return
			end
		end
...
--print ("in front:", nod.name, pos.y + 0.5)
	if self.walk_chance == 0 or minetest.registered_items[nod.name].walkable then
...
local follow_holding = function(self, clicker)
	...
	if rings.players[name] and
			rings.players[name] == "rings:invisibility" then
		return false
	end
...
local breed = function(self)
	...
	if self.horny == true and self.hornytimer < 240 then
		self.hornytimer = self.hornytimer + 1
		if self.hornytimer >= 240 then
			self.hornytimer = 0
			self.horny = false
	...
	if self.horny == true and self.hornytimer <= 40 then
		...
		for n = 1, #objs do
			...
			if ent and canmate == true and ent.horny == true and
					ent.hornytimer <= 40 then
				num = num + 1
			end
...
local replace = function(self, pos)
	if not mobs_griefing or not self.replace_rate or
			not self.replace_what or self.child == true or
			self.object:get_velocity().y ~= 0 or
			random(1, self.replace_rate) > 1 then
		return
	end
...
local day_docile = function(self)
	if self.docile_by_day == false then
		return false
	elseif self.docile_by_day == true and self.time_of_day > 0.2 and
			self.time_of_day < 0.8 then
		return true
	end
end
			...
			if self.pathfinding == 2 and mobs_griefing then
				...
				if s.y < p1.y then
					...
					if not minetest.is_protected(s, "") then
						...
						if node1 ~= "air" and node1 ~= "ignore"	and
								ndef1 and not ndef1.groups.level and
								not ndef1.groups.unbreakable and
								not ndef1.groups.liquid then
					...
					if not minetest.is_protected(p1, "") then
						...
						if node1 ~= "air" and node1 ~= "ignore"	and ndef1 and
								not ndef1.groups.level and
								not ndef1.groups.unbreakable and
								not ndef1.groups.liquid then
							minetest.add_item(p1, ItemStack(node1))
							minetest.set_node(p1, {name = "air"})
...
local general_attack = function(self)
	...
	if self.passive or self.state == "attack" or day_docile(self) then
		return
	end
	...
	for n = 1, #objs do
		...
		if objs[n]:is_player() then
			-- if mob not setup to attack then remove from list
	...
	for _, player in pairs(objs) do
		...
		if dist ~= 0 and dist < min_dist and
				line_of_sight(self, sp, p, 2) == true then
			min_dist = dist
			min_player = player
		end
	end
...
local follow_flop = function(self)
	...
	if (self.follow ~= "" or self.order == "follow") and
			not self.following and self.state ~= "attack" and
			self.state ~= "runaway" then
		local s = self.object:get_pos()
		local players = minetest.get_connected_players()
		for n = 1, #players do
			local pname = players[n]:get_player_name()
			local ring = rings.players[pname]
			if get_distance(players[n]:get_pos(), s) < self.view_range and
					not (rings.players[pname] and
					rings.players[pname] == "rings:invisibility") then
				self.following = players[n]
				break
			end
		end
	end
	if self.type == "npc" and self.order == "follow" and
			self.state ~= "attack" and self.owner ~= "" then
		-- npc stop following player if not owner
		if self.following and self.owner and
				self.owner ~= self.following:get_player_name() then
			self.following = nil
		end
	else
		-- stop following player if not holding specific item
		if self.following and self.following:is_player() and
				follow_holding(self, self.following) == false then
			self.following = nil
		end
	end
	-- follow that thing
...
local dogswitch = function(self, dtime)
	...
	if (self.dogshoot_switch == 1 and
			self.dogshoot_count > self.dogshoot_count_max) or
			(self.dogshoot_switch == 2 and
			self.dogshoot_count > self.dogshoot_count2_max) then
...
local do_states = function(self, dtime)
	...
	if self.state == "stand" then
		...
		if self.order ~= "stand" and self.walk_chance ~= 0 and
				self.facing_fence ~= true and
				random(1, 100) <= self.walk_chance and
				is_at_cliff(self) == false then
			set_velocity(self, self.walk_velocity)
			self.state = "walk"
			set_animation(self, "walk")
		end
	elseif self.state == "walk" then
		...
		if self.water_damage > 0 and self.lava_damage > 0 then
		...
		if lp then
			...
			if (self.lava_damage and
					minetest.registered_nodes[self.standing_in].groups.lava) or
					(self.water_damage and
					minetest.registered_nodes[self.standing_in].groups.water) then
		...
		if self.facing_fence == true or temp_is_cliff or random(1, 100) <= 30 then
			...
			set_velocity(self, self.walk_velocity)
			if flight_check(self) and self.animation and
					self.animation.fly_start and
					self.animation.fly_end then
	...
	elseif self.state == "runaway" then
		...
		if self.runaway_timer > 5 or is_at_cliff(self) or self.order == "stand" then
	...
	elseif self.state == "attack" then
		...
		local ring = rings.players[self.attack:get_player_name()]
		if dist > self.view_range or not self.attack or
			...
			--log(" ** stop attacking **", dist, self.view_range)
			self.state = "stand"
			set_velocity(self, 0)
			set_animation(self, "stand")
			self.attack = nil
		...
		if self.attack_type == "explode" then
			...
			if not self.v_start and dist <= self.reach and
					line_of_sight(self, s, p, 2) then
				self.v_start = true
				self.timer = 0
				self.blinktimer = 0
			...
			elseif self.allow_fuse_reset and self.v_start and
					(dist > self.reach or
					not line_of_sight(self, s, p, 2)) then
				self.v_start = false
				self.timer = 0
				self.blinktimer = 0
			...
			if self.v_start then
				...
				if self.timer > self.explosion_timer then
					...
					if minetest.find_node_near(pos, 1, {"group:water"}) or
		...
		elseif self.attack_type == "dogfight" or
				(self.attack_type == "dogshoot" and
				dogswitch(self, dtime) == 2) or
				(self.attack_type == "dogshoot" and
				dist <= self.reach and dogswitch(self) == 0) then
			...
			if self.path.following and self.path.way and
					self.attack_type ~= "dogshoot" then
...
--						end
		...
		elseif self.attack_type == "shoot" or
				(self.attack_type == "dogshoot" and dogswitch(self, dtime) == 1) or
				(self.attack_type == "dogshoot" and dist > self.reach and dogswitch(self) == 0) then
			p.y = p.y - .5
			s.y = s.y + .5
			local dist = get_distance(p, s)
			local vec = {
...
local mob_punch = function(self, hitter, tflp, tool_capabilities, dir)
	...
	if self.do_punch and self.do_punch(self, hitter, tflp, tool_capabilities, dir) == false then
		return
	end
...
--	print ("Mob Damage is", damage)
	...
	if damage >= 1 then
		...
		if weapon_def.sounds then
			...
			minetest.sound_play("mobs_punch", {
				object = self.object,
				max_hear_distance = 64
			})
		end
		...
		local hot = tool_capabilities and
				tool_capabilities.damage_groups and
				tool_capabilities.damage_groups.fire
	...
	if self.passive == false and
		...
		for n = 1, #objs do
			...
			if obj then
				...
				if obj.group_attack == true and
						obj.state ~= "attack" and
						obj.owner ~= name and
						obj.name == self.name then
...
local mob_activate = function(self, staticdata, def, dtime)
	...
	self.old_y = self.object:get_pos().y
	self.old_health = self.health
	self.sounds.distance = self.sounds.distance or 64
	...
	-- Trader if trader.
	self.tid = self.tid or nil
	self.inv = self.inv or nil
...
--		print ("standing in " .. self.standing_in)
	...
	local on_stepper = self.stepper or random()
	self.stepper = on_stepper + dtime
	...
	if undercrowd(pos, 32) > 3 then
		log("on_step: return, undercrowd(pos, 32) > 3")
		return
	end

	if minetest.find_node_near(pos, 8, "mobs:spawner") then
		log("on_step: return, found spawner")
		return
	end

	local node = minetest.get_node_or_nil(pos)
	if not node then
		log("on_step: return, node is nil")
		return
	end
	...
	node_name = node.name
	if node_name ~= "air" then
		log("on_step: return, node_name ~= air")
		return
	end

	local added = minetest.add_node(pos, {name = "mobs:spawner"})
	if not added then
		log("on_step: return, spawner not added")
		return
	else
		log("on_step: added spawner")
	end
...
function mobs:register_mob(name, def)
	...
	minetest.register_entity(name, {
		...
		physical = true,
		collisionbox = def.collisionbox or
		...
		follow = def.follow,
		jump = def.jump ~= false,
		jump_height = def.jump_height or 6, -- Vertical velocity!
		...
		runaway = def.runaway,
		runaway_timer = 0,
		pathfinding = false, --def.pathfinding,
		...
		dogshoot_count2_max = def.dogshoot_count2_max or
				(def.dogshoot_count_max or 5),
...
local count_mobs = function(pos, t)
	local num_type = 0
	local num_total = 0
	...
	for n = 1, #objs do
		if not objs[n]:is_player() then
			...
			if obj and obj.name and obj.name == t then
				num_type = num_type + 1
	...
	minetest.register_abm({
		...
		action = function(pos, node, active_object_count, active_object_count_wider)
			...
			--or count_mobs(pos, name) >= aoc then
				--log("--- too many entities", name, aoc, active_object_count_wider)
				return
			end
			...
			if day_toggle ~= nil then
				...
				if tod > 4500 and tod < 19500 then
					...
					if day_toggle == false then
						--log("--- mob needs night", name)
						return
					end
				else
					-- night time but mob wants day
					if day_toggle == true then
						--log("--- mob needs day", name)
						return
					end
				end
			end
			...
			if pos.y > max_height or pos.y < min_height then
				--log("--- height limits not met", name, pos.y)
				return
			end
			...
			if not light or light > max_light or light < min_light then
				--log("--- light limits not met", name, light)
				return
			end
			...
			for n = 0, height do
				...
				if minetest.registered_nodes[node_ok(pos2).name].walkable == true then
					--log("--- inside block", name, node_ok(pos2).name)
					return
				end
			end
			--]]
			...
			and minetest.is_protected(pos, "") then
				--log("--- inside protected area", name)
				return
			end
			--]]
			...
			if self.hit_player or self.hit_mob then
				for _,player in pairs(minetest.get_objects_inside_radius(pos, 1.0)) do
					if self.hit_player and player:is_player() then
						...
						self.object:remove() ; --log("hit player")
						return
					end

					local entity = player:get_luaentity()
					if entity and self.hit_mob and
							tostring(player) ~= self.owner_id and
							entity.name ~= self.object:get_luaentity().name then
...
function mobs:register_egg(mob, desc, background, addegg, no_creative)
	...
	minetest.register_craftitem(mob .. "_set", {
		...
		groups = {spawn_egg = 2, not_in_creative_inventory = 1, trade_value = 20},
		stack_max = 1,
